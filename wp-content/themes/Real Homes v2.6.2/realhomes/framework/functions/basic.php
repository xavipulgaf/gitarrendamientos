	...
	function inspiry_post_nav( $same_category = false )
	{
		...
		if ( ( 'true' == get_option( 'inspiry_property_prev_next_link' ) && is_singular( 'property' ) )
			...
			foreach ( $entries as $key => $entry )
			{
				...
				$icon = ( $key == 'prev' ) ?  'angle-left' : 'angle-right';

				?>
				<a class='inspiry-post-nav inspiry-post-<?php echo $key . ' ' . $class; ?>' href='<?php echo $link; ?>' >
					<span class='label'><i class="fa fa-<?php echo $icon; ?>"></i></span>
					<span class='entry-info-wrap'>
						<span class='entry-info'>
							<?php

								$eTitle = "<span class='entry-title'>{$the_title}</span>";

								if( $image ) {
									$eImg = "<span class='entry-image'>{$image}</span>";
								}
								echo ( $key == 'prev' ) ?  $eTitle.$eImg : $eImg.$eTitle;
							?>
							</span>
					</span>
				</a>
...
if ( ! function_exists( 'google_map_needed' ) ) {
	...
	function google_map_needed() {
		if ( is_page_template( 'template-contact.php' ) && ( get_option( 'theme_show_contact_map' ) == 'true' ) ) {
			...
			return true;
		} else if ( ( is_page_template( 'template-search-sidebar.php' ) || is_page_template( 'template-search-right-sidebar.php' ) ) && ( get_option( 'theme_search_module' ) == 'properties-map' ) ) {
			return true;
		} else if ( is_page_template( 'template-property-listing.php' ) || is_page_template( 'template-property-grid-listing.php' ) || is_tax( 'property-city' ) || is_tax( 'property-status' ) || is_tax( 'property-type' ) || is_tax( 'property-feature' ) || is_post_type_archive( 'property' ) ) {
